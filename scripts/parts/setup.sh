...
# Build JVM arguments dynamically based on configuration
JVM_ARGS="-Xms${XMS} -Xmx${XMX} --add-modules=jdk.incubator.vector"
JVM_ARGS="${JVM_ARGS} -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC"

# GC Threading configuration (parameterized)
JVM_ARGS="${JVM_ARGS} -XX:ParallelGCThreads=${PARALLEL_GC_THREADS} -XX:ConcGCThreads=${CONCURRENT_GC_THREADS}"

# Core GC tuning
JVM_ARGS="${JVM_ARGS} -XX:+AlwaysPreTouch -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15"
JVM_ARGS="${JVM_ARGS} -XX:G1MixedGCLiveThresholdPercent=90 -XX:MaxMetaspaceSize=1G -XX:G1RSetUpdatingPauseTimePercent=5 -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem"
JVM_ARGS="${JVM_ARGS} -XX:MaxTenuringThreshold=1 -Dusing.aikars.flags=https://mcflags.emc.gs -Daikars.new.flags=true -XX:G1NewSizePercent=30"
JVM_ARGS="${JVM_ARGS} -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=${G1_HEAP_REGION_SIZE} -XX:G1ReservePercent=2"
...
#Generated by flags.sh (mostly) - Performance tuning configurable via environment variables
